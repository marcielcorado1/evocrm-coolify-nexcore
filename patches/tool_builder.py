...
class ToolBuilder:
    ...
    def _create_http_tool(self, tool_config: Dict[str, Any]) -> FunctionTool:
        ...
        # ORACLE HOTFIX 2026-07-02: aceitar formato aninhado (parameters.*) E flat (topo)
        ...
        body_params = parameters.get("body_params") or tool_config.get("body_params") or {}
        ...
        def http_tool(**kwargs):
            try:
                ...
                # ORACLE HOTFIX 2026-07-02: values so viram query em metodos SEM corpo.
                # Em POST/PUT/PATCH os values (ex: url, type, caption) devem ir no BODY.
                if str(method).upper() in ("GET", "DELETE", "HEAD"):
        ...
        # ORACLE HOTFIX 2026-07-02: funcao com parametros NOMEADOS reais para o LLM
        # receber o schema. Obrigatorios primeiro (evita "non-default follows default").
        try:
            _pnames = []
            for _p in list(path_params.keys()) + list(query_params.keys()):
                _pnames.append((_p, True))
            for _p, _pc in body_params.items():
                _req = not (isinstance(_pc, dict) and not _pc.get("required", False))
                _pnames.append((_p, _req))
            _pnames = [x for x in _pnames if x[1]] + [x for x in _pnames if not x[1]]
            if _pnames:
                _sig_parts = [(_n if _r else _n + "=None") for _n, _r in _pnames]
                _call_src = ", ".join([_n + "=" + _n for _n, _ in _pnames])
                _src = (
                    "def _tool_fn(" + ", ".join(_sig_parts) + "):\n"
                    "    _kw = dict(" + _call_src + ")\n"
                ...
                _ns = {"_impl": http_tool}
                exec(_src, _ns)
                _tool_fn = _ns["_tool_fn"]
                _tool_fn.__name__ = name
                _tool_fn.__doc__ = http_tool.__doc__
                return FunctionTool(func=_tool_fn)
        except Exception as _e:
            logger.error("ORACLE fix: falha ao gerar assinatura de " + str(name) + ": " + str(_e))

...
# ORACLE HOTFIX 2026-07-02: model SQLAlchemy para as queries (o import acima e o schema Pydantic)
...
async def get_custom_tools(
    ...
    try:
        query = db.query(CustomToolModel)

        if search:
            search_filter = f"%{search}%"
            query = query.filter(
                CustomToolModel.name.ilike(search_filter)
                | CustomToolModel.description.ilike(search_filter)
            )

        if tags:
            # Filter by tags (JSON contains)
            for tag in tags:
                query = query.filter(CustomToolModel.tags.contains([tag]))

        return query.order_by(CustomToolModel.name).offset(skip).limit(limit).all()
